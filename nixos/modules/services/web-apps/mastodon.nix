  ...
  sidekiqUnits = lib.attrsets.mapAttrs' (name: processCfg:
    ...
    in {
      ...
      serviceConfig = {
        ...
        SyslogIdentifier = "mastodon-sidekiq-${name}";
        # System Call Filtering
...
in {
  ...
  options = {
    services.mastodon = {
      ...
      sidekiqProcesses = lib.mkOption {
        description = lib.mdDoc "How many Sidekiq processes should be used to handle background jobs, and which job classes they handle.";
        type = with lib.types; attrsOf (submodule {
          options = {
            jobClasses = lib.mkOption {
              type = listOf (enum [ "default" "push" "pull" "mailers" "scheduler" "ingress" ]);
              description = lib.mdDoc "If not empty, which job classes should be executed by this process. *Only one process should handle the 'scheduler' class.*";
            };
  ...
  config = lib.mkIf cfg.enable (lib.attrsets.recursiveUpdate {
    assertions = [
      {
